...
const UploadFiles = () => {
  ...
  const handleInsertData = () => {
    if (uploadedFiles.length === 0) {
      toast({
        title: t("common.error"),
        description: t(`upload.no${fileType}FilesToInsert`),
        variant: "destructive",
      });
      return;
    }
    ...
    const processAllFiles = async () => {
      ...
      try {
        for (const uploadedFile of uploadedFiles) {
          const fileData = await readSpreadsheetFile(uploadedFile.file);
          
          let processedData;
          if (fileType === "transactions") {
            processedData = processTransactionsFile(fileData);
            await saveTransactionsData(processedData, false);
          } else {
            processedData = processCategoriesFile(fileData);
            await saveCategoriesData(processedData, false);
          }
        }
